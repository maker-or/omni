...
#
# Accurate timing run:
#   DETAIL=0 RUNS=5 bash scripts/benchmark-shipped-startup.sh
#
# Detailed diagnostic run:
#   DETAIL=1 RUNS=5 bash scripts/benchmark-shipped-startup.sh
#
# Direct launch captures the shipped app's stdout/stderr, which may contain
# existing main-process lifecycle logs. It is useful diagnostically, but use
# the default "open" mode for Finder-like launch timing.
#
#   DETAIL=1 LAUNCH_MODE=direct RUNS=3 bash scripts/benchmark-shipped-startup.sh

set -u

APP_PATH="${APP_PATH:-/Applications/Pipper Code (Alpha).app}"
APP_NAME="${APP_NAME:-Pipper Code (Alpha)}"
RUNS="${RUNS:-5}"
TIMEOUT_SECONDS="${TIMEOUT_SECONDS:-30}"
DETAIL="${DETAIL:-0}"
LAUNCH_MODE="${LAUNCH_MODE:-open}"
OUTPUT_DIR="${OUTPUT_DIR:-$PWD/startup-benchmark-results}"

if [[ ! -d "$APP_PATH" ]]; then
  echo "App not found: $APP_PATH" >&2
  echo "Set APP_PATH to the shipped .app bundle." >&2
  exit 1
fi
...
capture_process_sample() {
  ...
  [[ -z "$pid" ]] && return 0
  {
    ...
    ps -p "$pid" -o pid=,ppid=,state=,%cpu=,rss=,thcount=,etime=,command=
...
wait_for_window() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ "$LAUNCH_MODE" == direct && -f "$run_dir/app.stdout.log" ]] &&
...
for ((run = 1; run <= RUNS; run++)); do
  ...
  if [[ "$DETAIL" == 1 ]]; then
    ...
    ps -axo pid=,ppid=,state=,%cpu=,rss=,thcount=,etime=,command= >"$run_dir/process-table-at-process-start.txt"
